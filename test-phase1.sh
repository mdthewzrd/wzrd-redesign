...
if npx tsc --noEmit remi/context/topic-manager.sh > /tmp/phase1-compile.log 2>&1; then
    ...
    if [ -f "/home/mdwzrd/wzrd-redesign/memory/unified-memory.ts" ]; then
...
if [ -f "/home/mdwzrd/wzrd-redesign/topics/registry.ts" ]; then
    test_pass "Topic registry file exists"
else
    test_fail "Topic registry file missing"
fi

if [ -f "/home/mdwzrd/wzrd-redesign/topics/config.yaml" ]; then
...
if [ -f "/home/mdwzrd/wzrd-redesign/models/router.ts" ]; then
    test_pass "Model router file exists"
else
    test_fail "Model router file missing"
fi

if [ -f "/home/mdwzrd/wzrd-redesign/models/config.yaml" ]; then
...
if [ -f "/home/mdwzrd/wzrd-redesign/cost/tracker.ts" ]; then
...
for file in "${memory_files[@]}"; do
    if [ -f "/home/mdwzrd/wzrd-redesign/$file" ]; then
...
if grep -q "CostTracker\|cost-tracker" /home/mdwzrd/wzrd-redesign/models/router.ts 2>/dev/null; then
...
if grep -q "topicRegistryPath\|TopicRegistry" /home/mdwzrd/wzrd-redesign/memory/unified-memory.ts 2>/dev/null; then
...
if [ -f "/home/mdwzrd/wzrd-redesign/configs/cost-tracker.json" ]; then
    budget=$(jq -r '.budget.daily_limit' /home/mdwzrd/wzrd-redesign/configs/cost-tracker.json)
...
if [ -f "/home/mdwzrd/wzrd-redesign/models/config.yaml" ]; then
    if grep -q "glm-4.7-flash" /home/mdwzrd/wzrd-redesign/models/config.yaml; then
        test_pass "GLM 4.7 Flash model configured"
    fi
    
    if grep -q "deepseek-v3.2" /home/mdwzrd/wzrd-redesign/models/config.yaml; then
        test_pass "DeepSeek V3.2 model configured"
    fi
    
    if grep -q "qwen3-coder" /home/mdwzrd/wzrd-redesign/models/config.yaml; then
...
if [ -f "/home/mdwzrd/wzrd-redesign/PHASE_1_CORE.md" ]; then
    test_pass "Phase 1 plan document exists"
else
    test_warn "Phase 1 plan document missing (continuing without it)"
fi

if [ -f "/home/mdwzrd/wzrd-redesign/PHASE_1_COMPLETE.md" ]; then
